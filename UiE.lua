-- Optimized Kavo UI Library - Very Smooth Performance
local Kavo = {}
...
function Kavo:DraggingEnabled(frame, parent)
    ...
    -- Optimized dragging with performance improvements
    ...
    input.InputChanged:Connect(function(inputObject)
        if inputObject == dragInput and dragging then
            ...
            -- Throttle to 60 FPS for smooth dragging
...
local function safeFileOperation()
    local success, result = pcall(function()
        if not pcall(function() readfile(Name) end) then
            writefile(Name, Services.HttpService:JSONEncode(SettingsT))
        end
        return Services.HttpService:JSONDecode(readfile(Name))
    end)
    return success and result or SettingsT
end

local Settings = safeFileOperation()
...
function Kavo:ToggleUI()
    local success = pcall(function()
        local screenGui = Services.CoreGui:FindFirstChild(LibName)
        if screenGui and screenGui.Enabled ~= nil then
...
function Kavo.CreateLib(kavName, themeList)
    ...
    -- Safe CoreGui/PlayerGui detection and setup
    local targetParent
    local success = pcall(function()
        targetParent = Services.CoreGui
    end)
    ...
    -- Safe parent assignment with error handling
    local parentSuccess = pcall(function()
        ScreenGui.Parent = targetParent
    end)
    
    if not parentSuccess then
        warn("Failed to parent ScreenGui, falling back to PlayerGui")
        ScreenGui.Parent = Services.Players.LocalPlayer:WaitForChild("PlayerGui")
    end
    
    -- Store UI reference safely
    ...
    -- Optimized close button with smooth animation
    ...
    function Kavo:ChangeColor(prope, color)
        ...
        if not colorSuccess then
            warn("Failed to change color for property:", prope)
        end
    end
    
    -- Cleanup function for theme connections
    ...
    getgenv().KavoCleanup = cleanupConnections
    local Tabs = {}
    ...
    function Tabs:NewTab(tabName)
        ...
        -- Optimized tab button click with smooth transitions
        tabButton.MouseButton1Click:Connect(function()
            local clickSuccess = pcall(function()
                ...
                -- Update tab button states with smooth transitions
                for i,v in next, tabFrames:GetChildren() do
                    if v:IsA("TextButton") then
                        local targetTextColor = themeList.TextColor
                        if themeList.SchemeColor == Color3.fromRGB(255,255,255) then
                            targetTextColor = Color3.fromRGB(255,255,255)
                        elseif themeList.SchemeColor == Color3.fromRGB(0,0,0) then
                            targetTextColor = Color3.fromRGB(0,0,0)
                        end
                ...
                -- Update active tab
                local activeTextColor = themeList.TextColor
                if themeList.SchemeColor == Color3.fromRGB(255,255,255) then
                    activeTextColor = Color3.fromRGB(0,0,0)
                elseif themeList.SchemeColor == Color3.fromRGB(0,0,0) then
        ...
        local tabThemeConnections = {}
        
        local function updateTabThemes()
            pcall(function()
                page.BackgroundColor3 = themeList.Background
        ...
        local tabConnection = run.Heartbeat:Connect(function()
            local currentTime = tick()
            if currentTime - lastThemeUpdate < updateInterval then
                return
            end
        ...
        table.insert(tabThemeConnections, tabConnection)
        ...
        coroutine.wrap(function()
            ...
            local function updateSectionFrame()
                ...
                UpdateSize()
            local Elements = {}
            -- Optimized button creation with safe image loading
            function Elements:NewButton(bname, tipINf, callback)
                ...
                -- Use safer image IDs or fallback to text
                local imageLoadSuccess = pcall(function()
                ...
                if not imageLoadSuccess then
                    ...
                    textLabel.Parent = viewInfo
                    textLabel.Size = UDim2.new(1, 0, 1, 0)
                    textLabel.BackgroundTransparency = 1
                    textLabel.Text = "?"
                    textLabel.TextColor3 = themeList.SchemeColor
