...
-- Utility and object management
local Utility = {}
local Objects = {}
local Connections = {}
-- Optimized dragging system with better performance
function Kavo:DraggingEnabled(frame, parent)
    ...
    -- Optimized drag update with throttling
    local lastUpdate = 0
    local UPDATE_INTERVAL = 1/60 -- 60fps cap
    ...
    local function updateDrag(input)
        ...
        if dragData.isDragging and input == dragData.dragInput then
            local delta = input.Position - dragData.mousePos  
    ...
    frame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            dragData.mousePos = input.Position
            dragData.framePos = parent.Position
    ...
    -- Optimized input connection
    dragData.connection = Services.UserInputService.InputChanged:Connect(updateDrag)
    
    -- Cleanup on frame destruction
    frame.AncestryChanged:Connect(function(_, parent)
        if not parent and dragData.connection then
            dragData.connection:Disconnect()
        end
    end)
end

-- Optimized tween utility with caching
local TweenCache = {}
function Utility:TweenObject(obj, properties, duration, ...)
    local key = tostring(obj) .. duration .. table.concat({...}, ",")
    
    if not TweenCache[key] then
        TweenCache[key] = TweenInfo(duration, ...)
    end
    
    local tween = Services.TweenService:Create(obj, TweenCache[key], properties)
    tween:Play()
    return tween
end

-- Cleanup unused tweens periodically
Services.RunService.Heartbeat:Connect(function()
    for key, info in pairs(TweenCache) do
        -- Clean up old cached tween infos (basic cleanup)
...
-- Configuration system with error handling
local SettingsT = {}
...
local function initConfig()
    if not safeFileOperation(readfile, Name) then
        safeFileOperation(writefile, Name, Services.HttpService:JSONEncode(SettingsT))
    end
    
    local configData = safeFileOperation(readfile, Name)
    if configData then
        Settings = Services.HttpService:JSONDecode(configData)
    else
        Settings = SettingsT
    end
end
...
-- Optimized UI toggle with error handling
function Kavo:ToggleUI()
    local playerGui = LocalPlayer.PlayerGui
    local gui = playerGui:FindFirstChild(LibName)
    
    if gui then
        gui.Enabled = not gui.Enabled
        return gui.Enabled
    else
        warn("UI not found: " .. LibName)
        return false
    end
end
...
function Kavo.CreateLib(kavName, themeList)
    ...
    close.MouseButton1Click:Connect(function()
        ...
        local closeTween = Services.TweenService:Create(close, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
            ImageTransparency = 1
        })
        closeTween:Play()
        
        closeTween.Completed:Connect(function()
            local shrinkTween = Services.TweenService:Create(Main, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    ...
    -- Optimized theme update system with throttling
    local lastThemeUpdate = 0
    local THEME_UPDATE_INTERVAL = 0.1 -- Update every 100ms
    
    local themeElements = {
        {Main, "BackgroundColor3", "Background"},
        {MainHeader, "BackgroundColor3", "Header"},
        {MainSide, "BackgroundColor3", "Header"},
        {coverup_2, "BackgroundColor3", "Header"},
    ...
    local function updateTheme()
        ...
        for _, element in ipairs(themeElements) do
            local obj, property, themeProperty = element[1], element[2], element[3]
            if obj and obj.Parent then
                obj[property] = themeList[themeProperty]
            end
        end
    end
    
    -- Use RunService for better performance
    table.insert(Connections, Services.RunService.Heartbeat:Connect(updateTheme))
    ...
    function Tabs:NewTab(tabName)
        ...
        -- Optimized size update with throttling
        local lastSizeUpdate = 0
        local SIZE_UPDATE_INTERVAL = 0.05 -- 20fps for size updates
        
        local function UpdateSize()
            local currentTime = tick()
            if currentTime - lastSizeUpdate < SIZE_UPDATE_INTERVAL then return end
            lastSizeUpdate = currentTime
            
            local contentSize = pageListing.AbsoluteContentSize
            
            local sizeTween = Services.TweenService:Create(page, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        ...
        -- Optimized theme update for tab elements
        local tabThemeElements = {
            {page, "BackgroundColor3", "Background"},
            {tabButton, "TextColor3", "TextColor"},
            {tabButton, "BackgroundColor3", "SchemeColor"}
        }
        
        local function updateTabTheme()
            for _, element in ipairs(tabThemeElements) do
                local obj, property, themeProperty = element[1], element[2], element[3]
                if obj and obj.Parent then
                    obj[property] = themeList[themeProperty]
                end
        ...
        table.insert(Connections, Services.RunService.Heartbeat:Connect(function()
            if math.random() < 0.1 then -- Update 10% of frames for performance
                updateTabTheme()
            end
        end))
		...
		sectionHead.Visible = true
            ...
            -- Optimized section theme updates
            local sectionThemeElements = {
            ...
            local function updateSectionTheme()
                for _, element in ipairs(sectionThemeElements) do
                    local obj, property, themeProperty = element[1], element[2], element[3]
                    if obj and obj.Parent then
                        obj[property] = themeList[themeProperty]
                    end
            ...
            table.insert(Connections, Services.RunService.Heartbeat:Connect(function()
                if math.random() < 0.05 then -- Update 5% of frames for performance
                    updateSectionTheme()
                end
            end))

            -- Optimized section frame updates
            ...
            function Elements:NewButton(bname,tipINf, callback)
                ...
                -- Optimized button theme updates
                local buttonThemeElements = {
                    {viewInfo, "ImageColor3", "SchemeColor"},
                    {Sample, "ImageColor3", "SchemeColor"},
                    {touch, "ImageColor3", "SchemeColor"},
                    {btnInfo, "TextColor3", "TextColor"}
                }
                ...
                local function updateButtonTheme()
                    ...
                    for _, element in ipairs(buttonThemeElements) do
                        local obj, property, themeProperty = element[1], element[2], element[3]
                        if obj and obj.Parent then
                            obj[property] = themeList[themeProperty]
                        end
                ...
                table.insert(Connections, Services.RunService.Heartbeat:Connect(function()
                    if math.random() < 0.03 then -- Update 3% of frames for performance
                        updateButtonTheme()
                    end
                end))
            ...
            function Elements:NewColorPicker(colText, colInf, defcolor, callback)
                ...
                -- Optimized theme handling with cached colors and throttled updates
                local lastThemeUpdate = 0
                local THEME_UPDATE_INTERVAL = 0.1
                ...
                local function updateThemeColors()
                    ...
                    local schemeColor = themeList.SchemeColor
                    local elementColor = themeList.ElementColor
                    local textColor = themeList.TextColor
                    
                    -- Batch color updates
                    touch.ImageColor3 = schemeColor
                    colorHeader.BackgroundColor3 = elementColor
                    togName.TextColor3 = textColor
                    viewInfo.ImageColor3 = schemeColor
                    colorInners.BackgroundColor3 = elementColor
                    toggleDisabled.ImageColor3 = schemeColor
                    toggleEnabled.ImageColor3 = schemeColor
                    togName_2.TextColor3 = textColor
                    Sample.ImageColor3 = schemeColor
                    
                    -- Calculate complex colors once
                    local modifiedSchemeColor = Color3.fromRGB(
                        schemeColor.r * 255 - 14,
                        schemeColor.g * 255 - 17,
                        schemeColor.b * 255 - 13
                    )
                    moreInfo.BackgroundColor3 = modifiedSchemeColor
                    moreInfo.TextColor3 = textColor
                end
                
                coroutine.wrap(function()
                    while true do
                        local now = tick()
                        if now - lastThemeUpdate >= THEME_UPDATE_INTERVAL then
                            lastThemeUpdate = now
                            updateThemeColors()
                        end
                        wait(THEME_UPDATE_INTERVAL)
                    end
                end)()
                updateSectionFrame()
                UpdateSize()
                ...
                -- Optimized color picker function with position caching
                ...
                -- Optimized rainbow toggle with performance improvements
                local RAINBOW_SPEED = 0.01
                local TOGGLE_TWEEN = TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut)
                local lastUpdate = 0
                
                local function togglerainbow()
                    if rainbow then
                        Services.TweenService:Create(toggleEnabled, TOGGLE_TWEEN, {
                            ImageTransparency = 1
                        }):Play()
                        rainbow = false
                        ...
                        -- Throttled rainbow update
                        rainbowconnection = rs.Heartbeat:Connect(function()
                            local now = tick()
                            if now - lastUpdate >= RAINBOW_SPEED then
                                lastUpdate = now
                                setrgbcolor({zigzag(counter),1,1})
                                counter = counter + RAINBOW_SPEED
                            end
                        end)
                    end
                end
